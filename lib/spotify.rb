module SpotifyService
  ...
  def SpotifyService.search_tracks(query)
    query_params = {
      q: query, 
      type: 'track'
    }.to_param

    uri = Uri.parse('https://api.spotify.com/v1/search?' + query_params)
    http = Net::HTTP.new(uri.host, uri.port)
    http.use_ssl = true

    track_request = Net::HTTP::Get.new(uri.request_uri)
    track_request["Authorization"] = "Bearer " + token
    track_response = http.request(track_request)
    
    return ActiveSupport::JSON.decode(track_response.body)
  end
end
